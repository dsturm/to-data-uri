...
function getFiles(document, textRange) {
    ...
    if (imageSrc.protocol && "file:" != imageSrc.protocol) {
        return Promise.reject({
            message: "Can't inline remote files yet."
        });
    }
    ...
    if (hasWorkspaceFolder()) {
        ...
        return Promise.resolve(
            vscode.workspace.findFiles(prefix + text, null, 10, null)
        );
    }
            ...
            getFiles(textRange.document, textRange.range)
                .then(function (uris) {
                    if (util.isArray(uris)) {
                        if (uris.length == 0) {
                            let message =
                                "Can't find the selected image file in your workspace.";
                            let fileName = textRange.document.getText(textRange.range);
                            if (fileName) {
                                message =
                                    "Can't find the file '" + fileName + "' in your workspace.";
                            }
                        ...
                        vscode.window
                            .showQuickPick(items, {
                                placeHolder: "Which file do you want to inline?"
                            })
                    ...
                    if (uris) {
                        ...
                        showMessage(
                            "Before running this command, select the value of the src attribute of a html img element.",
                            context
                        );
                    }
                })
